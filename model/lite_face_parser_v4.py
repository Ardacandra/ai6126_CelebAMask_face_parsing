...
class AttentionRefinementModule(nn.Module):
    def __init__(self, channels, reduction=4):
        super().__init__()
        self.pre = DepthwiseSeparableConv(channels, channels, kernel_size=3, stride=1)
        self.channel_gate = SqueezeExcitation(channels, reduction=reduction)
        self.spatial_gate = nn.Sequential(
            ConvBNAct(2, 1, kernel_size=3, stride=1, activation=False),
            nn.Sigmoid(),
        )
...
class TopDownFusion(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.low_proj = ConvBNAct(channels, channels, kernel_size=1, stride=1, padding=0)
        self.high_proj = ConvBNAct(channels, channels, kernel_size=1, stride=1, padding=0)
        self.fuse = DepthwiseSeparableConv(channels * 2, channels, kernel_size=3, stride=1)
...
class BottomUpFusion(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.high_proj = ConvBNAct(channels, channels, kernel_size=1, stride=1, padding=0)
        self.low_path = DepthwiseSeparableConv(channels, channels, kernel_size=3, stride=2)
        self.fuse = DepthwiseSeparableConv(channels * 2, channels, kernel_size=3, stride=1)
...
class EncoderBiRefine(nn.Module):
    def __init__(self, c1, c2, c3):
        super().__init__()
        self.refine1 = AttentionRefinementModule(c1)
        self.refine2 = AttentionRefinementModule(c2)
        self.refine3 = AttentionRefinementModule(c3)

        self.p1 = ConvBNAct(c1, c1, kernel_size=1, stride=1, padding=0)
        self.p2 = ConvBNAct(c2, c2, kernel_size=1, stride=1, padding=0)
        self.p3 = ConvBNAct(c3, c3, kernel_size=1, stride=1, padding=0)

        self.td2 = TopDownFusion(c2)
        self.td1 = TopDownFusion(c1)
        self.bu2 = BottomUpFusion(c2)
        self.bu3 = BottomUpFusion(c3)

        self.p2_from3 = ConvBNAct(c3, c2, kernel_size=1, stride=1, padding=0)
        self.p1_from2 = ConvBNAct(c2, c1, kernel_size=1, stride=1, padding=0)
        self.p1_to2 = ConvBNAct(c1, c2, kernel_size=1, stride=1, padding=0)
        self.p2_to3 = ConvBNAct(c2, c3, kernel_size=1, stride=1, padding=0)

    def forward(self, x1, x2, x3):
        p1 = self.p1(self.refine1(x1))
        p2 = self.p2(self.refine2(x2))
        p3 = self.p3(self.refine3(x3))

        td2 = self.td2(p2, self.p2_from3(p3))
        td1 = self.td1(p1, self.p1_from2(td2))

        bu2 = self.bu2(td2, self.p1_to2(td1))
        bu3 = self.bu3(p3, self.p2_to3(bu2))

        return td1, bu2, bu3


class LiteFaceParserV4(nn.Module):
    def __init__(
        self,
        num_classes=19,
        ...
        self.encoder_refine = EncoderBiRefine(c1, c2, c3)
    ...
    def forward(self, x):
        ...
        r1, r2, r3 = self.encoder_refine(x1, x2, x3)

        d3 = self.decode3(r3, context)
        d2 = self.decode2(r2, d3)
        d1 = self.decode1(r1, d2)
